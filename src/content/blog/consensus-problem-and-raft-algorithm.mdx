...
- `etcd` được xây dựng dựa trên thuật toán đồng thuận Raft.
...
trong hệ thống.
<Figure src={raftStartVoting} alt="">Ví dụ S2 timeout trước rồi chuyển mình sang Candidate, sau đó gửi request đén các node còn lại</Figure>
...
<Figure src={raftReplicatedLogSuccessAfterRestart} alt="">Cho đến khi S4 re-start lại, lúc này các node replicate entry thành công và phản hồi về S2, S2 mới gửi response về cho client</Figure>
